...
import {
  collection, getDocs, addDoc, doc, getDoc, updateDoc, deleteDoc, serverTimestamp
} from "https://www.gstatic.com/firebasejs/10.13.1/firebase-firestore.js";
import { onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/10.13.1/firebase-auth.js";
import { logAction } from "./logger.js";
import { esc, checkIsAdmin, renderUserBadge } from "./common.js";
...
function renderPartners(){
  ...
  partnersTable.innerHTML = list.map(p => `
    <tr>
      <td>${esc(p.name || "-")}</td>
      <td><span class="badge ${p.active ? "badge-yes" : "badge-no"}">${p.active ? "Oui" : "Non"}</span></td>
...
function renderMembers(){
  ...
  membersTable.innerHTML = OPEN_MEMBERS.map(m => `
    ...
    `).join("");
  membersTable.querySelectorAll("[data-edit]").forEach(btn=>{ btn.addEventListener("click", ()=> openMemberModal(btn.getAttribute("data-edit"))); });
  membersTable.querySelectorAll("[data-del]").forEach(btn=>{ btn.addEventListener("click", ()=> deleteMember(btn.getAttribute("data-del"))); });
}
...
pmSave?.addEventListener("click", async ()=>{
  ...
  try {
    await updateDoc(doc(db, "partners", OPEN_PARTNER_ID), { name, nameKey: name.toLowerCase(), active: pmActive.checked, updatedAt: serverTimestamp() });
...
pmDelete?.addEventListener("click", async ()=>{
  if(!OPEN_PARTNER_ID || !confirm("Supprimer ce partenaire ?")) return;
  try {
    await deleteDoc(doc(db, "partners", OPEN_PARTNER_ID));
    await logAction("PARTENAIRE_SUPPR", `Suppression partenaire ${OPEN_PARTNER_ID}`);
    hideAllModals(); await loadPartners();
  } catch (e) { console.error(e); alert("Erreur suppression."); }
});

addPartnerBtn?.addEventListener("click", () => { apmName.value = ""; addPartnerModal.classList.remove("hidden"); });
apmSave?.addEventListener("click", async () => {
  const name = apmName.value.trim();
  if (!name) return alert("Le nom est obligatoire.");
  try {
    await addDoc(collection(db, "partners"), { name, nameKey: name.toLowerCase(), active: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
...
function openMemberModal(memberId = null) {
  if (memberId) {
    ...
    if (!m) return;
    mmTitle.textContent = "Modifier Membre"; mmId.textContent = m.id;
    mmFullName.value = m.fullName || ""; mmClientId.value = m.clientId || ""; mmRate.value = m.rate || 0;
  } else {
    mmTitle.textContent = "Ajouter Membre"; mmId.textContent = "";
    mmFullName.value = ""; mmClientId.value = ""; mmRate.value = 0;
  }
...
mmSave?.addEventListener("click", async () => {
  ...
  try {
    const data = { fullName, clientId: clientId || null, rate, updatedAt: serverTimestamp() };
    if (mId) {
      await updateDoc(doc(db, "partners", OPEN_PARTNER_ID, "members", mId), data);
      await logAction("PARTENAIRE_MEMBRE_MODIF", `Modif membre ${mId}`);
    } else {
      data.createdAt = serverTimestamp();
      await addDoc(collection(db, "partners", OPEN_PARTNER_ID, "members"), data);
      await logAction("PARTENAIRE_MEMBRE_AJOUT", `Ajout membre`);
    }
    hideAllModals(); await loadPartners(); await loadMembers(OPEN_PARTNER_ID);
    partnerModal.classList.remove("hidden");
  } catch (e) { console.error(e); alert("Erreur sauvegarde membre."); }
});
...
async function deleteMember(memberId){
  ...
  try {
    ...
    await logAction("PARTENAIRE_MEMBRE_SUPPR", `Suppression membre ${memberId}`);
    await loadPartners(); await loadMembers(OPEN_PARTNER_ID);
  } catch (e) { console.error(e); alert("Erreur suppression membre."); }
}

onAuthStateChanged(auth, async (user) => {
  if (!user) { window.location.href = "pdm-staff.html"; return; }
  if (!(await checkIsAdmin(user.uid))) {
    document.body.innerHTML = `<div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000; color:#fff; font-family:sans-serif;"><h1 style="color:var(--accent-gold);">Accès Refusé</h1><p>Permissions insuffisantes.</p><button onclick="window.location.href='dashboard.html'" class="btn btn-gold" style="margin-top:20px;">Retour Dashboard</button></div>`;
  } else {
...
logoutBtn?.addEventListener("click", async () => { await signOut(auth); window.location.href = "pdm-staff.html"; });
